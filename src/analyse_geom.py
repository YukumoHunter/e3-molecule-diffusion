...
def analyse_geom_stability():
    ...
    atomic_nb_list = jnp.array(dataset_info['atomic_nb'])[None, :].astype(jnp.int32)
    num_stable_mols = 0
    num_mols = 0
    ...
    def check_molecule_stability(i, p, at_types):
        p = jnp.array(p)
        at_types = jnp.array(at_types)[:, None]
        one_hot = jnp.equal(at_types, atomic_nb_list).astype(jnp.int32)
        at_types = jnp.argmax(one_hot, axis=1)     # Between 0 and 15

        mol_is_stable, num_stable_atoms, num_atoms = check_stability(p, at_types, dataset_info)
        return mol_is_stable, num_stable_atoms, num_atoms

    def process_data(i, formatted_data, acc):
        mol_is_stable, num_stable_atoms, num_atoms = check_molecule_stability(i, all_positions_split[i], all_atom_types_split[i])

        new_formatted_data = formatted_data + [(all_positions_split[i], all_atom_types_split[i])]
        new_acc = (acc[0] + mol_is_stable, acc[1] + num_stable_atoms, acc[2] + num_atoms)
        
        return new_formatted_data, new_acc

    initial_data = ([], (0, 0, 0))
    formatted_data, (num_stable_mols, num_stable_atoms_total, num_atoms_total) = lax.scan(process_data, initial_data, jnp.arange(len(all_positions_split)))

    num_mols = len(all_positions_split)
...
def debug_geom_stability(num_atoms=100000):
    ...
    for p, at_types in zip(all_positions_split, all_atom_types_split):
        p, at_types = process_data(p, at_types)
