...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def sample_p_xh_given_z0(self, z0, node_mask, edge_mask, context, fix_noise=False):
        ...
        h_int = z0[:, :, -1:] if self.include_charges else jnp.zeros((z0.shape[0], z0.shape[1], 0))
    ...
    def log_pxh_given_z0_without_constants(
        ...
        z_h_int = z_t[:, :, -1:] if self.include_charges else jnp.zeros((z_t.shape[0], z_t.shape[1], 0))
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always):
        ...
        t_int = random.randint(
            key=random.PRNGKey(0), shape=(x.shape[0],), minval=lowest_t, maxval=self.T + 1, dtype=jnp.float32)
        s_int = t_int - 1
