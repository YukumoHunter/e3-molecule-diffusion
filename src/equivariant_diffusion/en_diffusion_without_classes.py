...
def positive_linear(params, inputs):
    ...
    return (
        jnp.dot(inputs, positive_weight.T) + bias
        if bias is not None
        else jnp.dot(inputs, positive_weight.T)
    )


def init_positive_linear(
    rng, in_features, out_features, bias=True, weight_init_offset=-2
):
...
def init_gamma_network_params(
    rng, in_features=1, hidden_size1=1, hidden_size2=1024, out_features=1
):
    l1_params = init_positive_linear(
        rng, in_features=in_features, out_features=hidden_size1
    )
    l2_params = init_positive_linear(
        rng, in_features=hidden_size1, out_features=hidden_size2
    )
    l3_params = init_positive_linear(
        rng, in_features=hidden_size2, out_features=out_features
    )
    gamma_0 = -5.0
    gamma_1 = 10.0
...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def sample_p_xh_given_z0(self, z0, node_mask, edge_mask, context, fix_noise=False):
        ...
        # Assuming F.one_hot is a utility function, you would need to define an equivalent function in JAX if it's not available.

        h_cat = F.one_hot(jnp.argmax(h_cat, axis=2), self.num_classes) * node_mask
