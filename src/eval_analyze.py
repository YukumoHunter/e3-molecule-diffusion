...
def analyze_and_save(args, eval_args, generative_model,
    ...
    for i in range(int(n_samples/batch_size)):
        ...
        one_hot, charges, x, node_mask = sample(
            args, generative_model, dataset_info, prop_dist=prop_dist, nodesxsample=nodesxsample)
    ...
    molecules = {key: jnp.concatenate(molecules[key], axis=0) for key in molecules}
...
def test(args, flow_dp, nodes_dist,  dtype, loader, partition='Test', num_passes=1):
    flow_dp.eval()
    nll_epoch = 0
    n_samples = 0
    for pass_number in range(num_passes):
        for i, data in enumerate(loader):
            ...
            if len(args.conditioning) > 0:
                context = prepare_context(args.conditioning, data)
...
def main():
    ...
    # Load model
    generative_model, nodes_dist, prop_dist = get_model(args, dataset_info, dataloaders['train'])
    if prop_dist is not None:
        property_norms = compute_mean_mad(dataloaders, args.conditioning, args.dataset)
        prop_dist.set_normalizer(property_norms)

    fn = 'generative_model_ema.npy' if args.ema_decay > 0 else 'generative_model.npy'
    flow_state_dict = np.load(join(eval_args.model_path, fn), allow_pickle=True)
    generative_model = jax.tree_map(jnp.array, flow_state_dict)

    # Analyze stability, validity, uniqueness and novelty
    stability_dict, rdkit_metrics = analyze_and_save(
        args, eval_args, generative_model, nodes_dist,
    ...
    val_nll = test(args, generative_model, nodes_dist, dtype,
                   dataloaders[val_name],
                   partition='Val')
    print(f'Final val nll {val_nll}')
    test_nll = test(args, generative_model, nodes_dist, dtype,
                    dataloaders['test'],
