...
class ProcessedDataset(Dataset):
    ...
    def __init__(self, data, included_species=None, num_pts=-1, normalize=True, shuffle=True, subtract_thermo=True):
        ...
        if shuffle:
            self.perm = jrandom.permutation(jrandom.PRNGKey(0), len(data['charges']))[:self.num_pts]
        else:
            self.perm = None

    def calc_stats(self):
        self.stats = {key: (val.mean(), val.std()) for key, val in self.data.items() if val.dim() == 1 and val.is_floating_point()}
