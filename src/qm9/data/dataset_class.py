...
class ProcessedDataset(Dataset):
    ...
    def __init__(
        ...
        if included_species is None:
            included_species = jnp.unique(self.data["charges"], sorted=True)
        ...
        if subtract_thermo:
            ...
            for key in thermo_targets:
                data[key] -= data[key + "_thermo"].astype(data[key].dtype)

        self.included_species = included_species

        self.data["one_hot"] = jnp.expand_dims(
            self.data["charges"], -1
        ) == jnp.expand_dims(jnp.expand_dims(included_species, 0), 0)
        ...
        if shuffle:
            self.perm = jrandom.permutation(jrandom.PRNGKey(0), len(data["charges"]))[
                : self.num_pts
            ]
        else:
            self.perm = None
    ...
    def calc_stats(self):
        self.stats = {
            ...
            if val.ndim == 1 and jnp.issubdtype(val, jnp.floating)
        }
