...
#Removed device
def train(model, epoch, loader, mean, mad, property, partition='train', optimizer=None, lr_scheduler=None, log_interval=20, debug_break=False):
    if partition == 'train':
        lr_scheduler.step()
    ...
    for i, data in enumerate(loader):
        ...
        edges = prop_utils.get_adj_matrix(n_nodes, batch_size)
        label = data[property]
...
#Removed device
def test(model, epoch, loader, mean, mad, property, log_interval, debug_break=False):
    return train(model, epoch, loader, mean, mad, property, partition='test', log_interval=log_interval, debug_break=debug_break)


def get_model(args):
    if args.model_name == 'egnn':
        model = EGNN(hidden_dim = args.nf, out_dim = 1, num_layers = args.n_layers, act_fn = nn.silu)
                # EGNN(in_node_nf=5, in_edge_nf=0, hidden_nf=args.nf, device=args.device, n_layers=args.n_layers,coords_weight=1.0, attention=args.attention, node_attr=args.node_attr)
    
    #Assume we wont use these
...
if __name__ == "__main__":
    ...
    for epoch in range(0, args.epochs):
        train(model, epoch, dataloaders['train'], mean, mad, args.property,  partition='train', optimizer=optimizer, lr_scheduler=lr_scheduler, log_interval=args.log_interval)
        if epoch % args.test_interval == 0:
            val_loss = train(model, epoch, dataloaders['valid'], mean, mad, args.property,  partition='valid', optimizer=optimizer, lr_scheduler=lr_scheduler, log_interval=args.log_interval)
            test_loss = test(model, epoch, dataloaders['test'], mean, mad, args.property,  log_interval=args.log_interval)
