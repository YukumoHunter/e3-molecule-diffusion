...
def sample(
    ...
    diag_mask = ~jnp.eye(edge_mask.shape[1], dtype=jnp.bool_).reshape(
        1, edge_mask.shape[1], edge_mask.shape[1]
    )
    edge_mask *= diag_mask
...
def sample_sweep_conditional(
    args, generative_model, dataset_info, prop_dist, n_nodes=19, n_frames=100
):
